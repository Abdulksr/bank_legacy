...
> **Configuración S3 (Arquitectura Escalable y Tolerante a Fallos):** Se implementó una arquitectura Master/Worker mediante un `CsvPartitioner` dinámico. El `partitionTaskExecutor` distribuye particiones utilizando un Thread Pool optimizado (`corePoolSize(5)`, `queueCapacity(10)`). La tolerancia a fallos ("Dirty Data") se gestiona mediante una Skip Policy global con un límite de 500 omisiones por partición, capturando excepciones de parseo y validación de negocio. Adicionalmente, se configuró una llave única compuesta (cuenta, año) para preservar la integridad referencial y prevenir sobreescrituras (idempotencia) en los Estados de Cuenta.
