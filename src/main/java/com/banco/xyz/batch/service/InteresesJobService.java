...
public class InteresesJobService {
    ...
    public JobExecution ejecutar() {
        try {
            JobParametersBuilder parametersBuilder = new JobParametersBuilder(jobExplorer)
                    .getNextJobParameters(interesesJob);
            return jobLauncher.run(interesesJob, parametersBuilder.toJobParameters());
        } catch (JobExecutionAlreadyRunningException | JobRestartException
                | JobInstanceAlreadyCompleteException | JobParametersInvalidException ex) {
            log.error("No fue posible iniciar el Job interesesJob", ex);
            throw new InteresesJobLauncherException(
                    "No fue posible iniciar el Job interesesJob: " + ex.getMessage(),
                    ex);
        }
    }
}
